...
lines = open('playlist.txt').readlines()
random.shuffle(lines)
open('shuffledPlaylist.txt', 'w').writelines(lines)

player.loadlist("shuffledPlaylist.txt")
led01.on()
...
while True:
    ...
    if metadados != None and posicao != None:
        lcd.clear()
        tituloCompleto = player.metadata["Title"]
        if len(tituloCompleto) > 16:
            i += 1
            if(i >= len(tituloCompleto)):
                i = 0
        else:
            i = 0
